...
def _(
    ...
    if not plotly_available:
        _runtime_notes.append(
            mo.callout(
                mo.md(
                    "**Plotly non disponibile in WASM**: i grafici interattivi sono disattivati, "
                    "ma la simulazione e le statistiche restano disponibili."
                ),
                kind="warn",
            )
        )
...
def _(
    ...
    if not plotly_available or go is None:
        _out = mo.vstack([
            mo.callout(
                mo.md("Grafico traiettorie non disponibile (Plotly mancante nel runtime web)."),
                kind="warn",
            ),
            mo.md(
                f"**alpha taMSD** = {alpha_ta:.3f} ± {alpha_ta_std:.3f}  \n"
                f"**alpha eaMSD** = {alpha_ea:.3f}"
            ),
        ])
    else:
...
def _(
    alpha_ea, alpha_ta, alpha_ta_std,
    eamsd, eb,
    go, model, mo, np, plotly_available, t_lags, tamsd, tamsd_mean,
):
    if not plotly_available or go is None:
        _out = mo.vstack([
            mo.callout(
                mo.md("Grafici MSD/EB non disponibili (Plotly mancante nel runtime web)."),
                kind="warn",
            ),
            mo.md(
                f"**alpha taMSD** = {alpha_ta:.3f} ± {alpha_ta_std:.3f}  \n"
                f"**alpha eaMSD** = {alpha_ea:.3f}  \n"
                f"**EB medio** = {float(np.nanmean(eb)):.3f}"
            ),
        ])
    else:
